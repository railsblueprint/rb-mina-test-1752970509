...
set :application, "mina_test_509"
set :application_name, "mina_test_509"  # Required for mina-puma-nginx gem
set :repository, 'git@github.com:railsblueprint/rb-mina-test-1752970509.git'
...
# Set UTF-8 locale for SSH to prevent encoding issues
set :ssh_options, lambda {
  options = {}
  options[:keys] = [fetch(:identity_file)] if fetch(:identity_file)
  options[:forward_agent] = true
  options[:user_known_hosts_file] = '/dev/null'
  options[:verify_host_key] = :never
  options[:send_env] = %w[LANG LC_CTYPE LC_ALL]
  options
}
